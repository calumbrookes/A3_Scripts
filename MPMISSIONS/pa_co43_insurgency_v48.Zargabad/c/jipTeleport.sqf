...
if (lastSquadTP+120>diag_tickTime) then {
hintsilent format ["Squad teleport will be available in %1 seconds",ceil(lastSquadTP+120 - diag_tickTime)];
} else {
...
if (_leader == player) then {
{
	...
	_bguy = _x;
	{
		if ((side group _x == east || side group _x == resistance) && alive _x) then {//check for enemies near player
			if (_x distance _bguy < 60) then {
	...
	{
		if ((side group _x == east || side group _x == resistance) && alive _x) then {//check for enemies near player
			if (_x distance _leader < 60) then {
				hintsilent "Enemies nearby, try again later"; 
				breakTo "main";
			};
		};
