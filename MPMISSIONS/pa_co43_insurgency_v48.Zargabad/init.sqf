...
if (!hasInterface || isServer) then {
	mccfr_make_array_grps 		= compile preProcessFileLineNumbers format ["mccfr\mccfr_make_array_grps.sqf",""];
	setviewdistance 500; //dist for player caching
};

if(isServer) then 
{
  execVM "c\Radiobox.sqf";
  []spawn {
	 sleep 10;
  ...
  if(isnil "Wtickets") then {Wtickets = 100; publicvariable "Wtickets";};
	 ...
	 ["GrpNATO_A1"]  spawn cb_fnc_eMarkerSend;
	 ["GrpNATO_B1"]  spawn cb_fnc_eMarkerSend;
	 ["GrpNATO_C1"]  spawn cb_fnc_eMarkerSend;
	 ["GrpNATO_D1"]  spawn cb_fnc_eMarkerSend;
	 ["OPFORguy2"]  spawn cb_fnc_eMarkerSend;
	 ["OPFORguy3"]  spawn cb_fnc_eMarkerSend;
	 ["OPFORguy4"]  spawn cb_fnc_eMarkerSend;
	 ["OPFORguy1"]  spawn cb_fnc_eMarkerSend;
	 
	// player caching server update pos
	//[{[] call cb_fnc_playerCachingServer;}, 6] call CBA_fnc_addPerFrameHandler;
...
if (!isdedicated && hasInterface) then {
	...
	/* 
	//setting group IDs here.. didnt work because it only set own group, not others.
	if (player == leader group player) then {
	if ( (vehicleVarName player) find "ASL" > 0) then {
		//{[[player], {GrpNATO_A1 = group (_this select 0);}] } remoteExec ["bis_fnc_call", 0, "Agroup"]; 
		GrpNATO_A1 = group PLAYER;
		publicVariable "GrpNATO_A1";
	};

	if ( (vehicleVarName player) find "BSL" > 0) then {
		GrpNATO_B1 = group PLAYER;
		publicVariable "GrpNATO_B1";
	};

	if ( (vehicleVarName player) find "CSL" > 0) then {
		GrpNATO_C1 = group PLAYER;
		publicVariable "GrpNATO_C1";
	};

	if ( (vehicleVarName player) find "DSL" > 0) then {
		GrpNATO_D1 = group PLAYER;
		publicVariable "GrpNATO_D1";
	};

	if ( (vehicleVarName player) find "CO" > 0) then {
		GrpNATO_CO = group PLAYER;
		publicVariable "GrpNATO_CO";
	};

	if ( (vehicleVarName player) find "CSAT" > 0) then {
		GrpCSAT_A1 = group PLAYER;
		publicVariable "GrpCSAT_A1";
	};
	};*/
	...
	[]spawn{
		...
		if (side group player == west) then {
			_sNeeds = str player in ["UnitNATO_CO_P1","UnitNATO_CO_P2","UnitNATO_CO_P3","UnitNATO_CO_PC","UnitNATO_CO","UnitNATO_CO_EN","UnitNATO_CO_FAC"];
			[{[_this select 0] call cb_fnc_playerCachingW;}, 6, _sNeeds] call CBA_fnc_addPerFrameHandler;
		} else {
				[{[] call cb_fnc_playerCachingE;}, 6] call CBA_fnc_addPerFrameHandler;
		};
			
		//player loop (obj mapmarkers)
		waituntil {sleep 2; !isnil"activeTowns"};
		sleep 15;
		[{[] call cb_fnc_playerLoop;}, 30, []] call CBA_fnc_addPerFrameHandler;
	};
	...
	if (side player == east) then {
		...
		"mkr_nogo3" setMarkerSizeLocal [850,850]; 
		...
		player addeventhandler ["Killed",{_this select 0 execVM "c\opforRespawnnew.sqf"}]; 
		//player addAction ["Spawn Quad", "c\spawnquad.sqf"];

		//disable ACE Switchunits for now
		/*
		//never uncomment: this setVariable ["ACE_CanSwitchUnits", true];
