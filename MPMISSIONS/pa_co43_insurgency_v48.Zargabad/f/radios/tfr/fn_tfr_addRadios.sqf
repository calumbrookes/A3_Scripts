...
if(_typeOfUnit != "NIL") then {
  ...
  if(!f_radios_settings_tfr_disableRadios) then {
	...
	[_unit]spawn{
		...
		waituntil {sleep 1; call TFAR_fnc_haveSWRadio;};
		_curCH = call TFAR_fnc_activeSwRadio;
		
	  //set radio CH
	  	if ( (vehicleVarName _unit) find "ASL" > 0) then {
		 [_curCH, 0] call TFAR_fnc_setSwChannel;
		};

		if ( (vehicleVarName _unit) find "BSL" > 0) then {
		 [_curCH, 1] call TFAR_fnc_setSwChannel;
		};

		if ( (vehicleVarName _unit) find "CSL" > 0) then {
		 [_curCH, 2] call TFAR_fnc_setSwChannel;
		};

		if ( (vehicleVarName _unit) find "DSL" > 0) then {
		 [_curCH, 3] call TFAR_fnc_setSwChannel;
		};

		if ( (vehicleVarName _unit) find "CO" > 0) then {
		 [_curCH, 4] call TFAR_fnc_setSwChannel;
		};
	  };
  };
