...
_per_fuel_time = if (count _this > 6) then { _this select 6 } else { 0.2 };
_per_repair_time = if (count _this > 7) then { _this select 7 } else { 0.4 };
...
//custom assigngear
_oVehicle setVariable ["f_var_assignGear_done",false,true];
_vehGear = _oVehicle getvariable "f_var_assignGear";
if (isnil "_vehGear") then {_oVehicle setVariable ["f_var_assignGear_done",true,true];} else {
[_vehGear,_oVehicle] call f_fnc_assignGear; 
};
...
if ( (str _list) == "[]" or _currentDist > _maxDistance or (getPos _oVehicle select 2) > _maxHeight or (str _oVehicle) == "any") exitWith 
{
	sleep 1;
...
if (_abort == 0) then
{
    ...
    if (_turretMagazinesCount > 0 ) then
    {
        _mag = 0;
        {
            ...
            _oVehicle removeMagazines (_turretMags select _mag);
            _oVehicle addMagazines [(_turretMags select _mag),1];
            _serviceEta = _serviceEta - _per_magazine_time;
		...
		if (_oVehicle iskindof "UK3CB_BAF_Jackal2_L2A1_D" || _oVehicle iskindof "UK3CB_BAF_Jackal2_L2A1_W"  || _oVehicle iskindof  "UK3CB_BAF_Coyote_Passenger_L111A1_W" || _oVehicle iskindof  "UK3CB_BAF_Coyote_Passenger_L111A1_D") then { 
		[_oVehicle]spawn{
			waitUntil{((_this select 0) getVariable ["f_var_assignGear_done", false])};
			(_this select 0) addMagazineCargoGlobal ["UK3CB_BAF_150Rnd_762x51_Box", 2]; //commander slot
			(_this select 0) addMagazineCargoGlobal ["UK3CB_BAF_100Rnd_127x99_Box", 3]; //hmg gunner slot 
			};
		};

		if (_oVehicle iskindof "UK3CB_BAF_Jackal2_GMG_D" || _oVehicle iskindof "UK3CB_BAF_Jackal2_GMG_W" || _oVehicle iskindof "UK3CB_BAF_Coyote_Passenger_L134A1_W" || _oVehicle iskindof "UK3CB_BAF_Coyote_Passenger_L134A1_D") then { 
		[_oVehicle]spawn{
			waitUntil{((_this select 0) getVariable ["f_var_assignGear_done", false])};
			(_this select 0) addMagazineCargoGlobal ["UK3CB_BAF_150Rnd_762x51_Box", 2]; //commander slot
			(_this select 0) addMagazineCargoGlobal ["UK3CB_BAF_32Rnd_40mm_G_Box", 2]; //gmg gunner slot 
			};
		};
		
    };
	if (_oVehicle iskindof "B_Heli_Light_01_armed_F" || _oVehicle iskindof "B_Heli_Light_01_F") then {
	//_oVehicle addweapon "CMFlareLauncher";  
	_oVehicle removemagazine "168Rnd_CMFlare_Chaff_Magazine";  
	_oVehicle addMagazine "168Rnd_CMFlare_Chaff_Magazine";  
	};
