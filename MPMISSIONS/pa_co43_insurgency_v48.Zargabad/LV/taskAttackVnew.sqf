...
_wpTowns = if(count _this > 0)then{_this select 0;}; //_cPos
_wpTarget = if(count _this > 1)then{_this select 1;}; //_cPos
...
_wpTowns = _wpTowns call BIS_fnc_arrayShuffle;
{
	if (!([_x, 300] call f_fnc_nearBPlayer)) ExitWith {
	_centerPos = getmarkerpos (_x call BIS_fnc_selectRandom);
	};
}foreach _wpTowns;

if (isnil "_centerPos") then {
	_centerPos = getmarkerpos (_wpTowns call BIS_fnc_selectRandom);
};
...

{
	if (_x == leader (group _x)) then{
		_firstWP = (_wpTowns call BIS_fnc_selectRandom);
		_wpTowns = _wpTowns - [_firstWP];
		_firstWP = getmarkerpos _firstWP;
		_theWP = (group _x) addwaypoint [_firstWP,0];
		_theWP setWaypointTimeout [30, 60, 90];
		//group _x) setCurrentWaypoint [(group _x),1];
		
		while {count _wpTowns > 0 && count waypoints (group _x)<5} do {
			_tempWP = (_wpTowns call BIS_fnc_selectRandom);
			//hint format ["%1\n\n%2",_wpTowns,_tempWP];
			_wpTowns = _wpTowns - [_tempWP];
			_tempWP = getmarkerpos _tempWP;
			_theWP = (group _x) addwaypoint [_tempWP,0];
			_theWP setWaypointTimeout [30, 60, 90];
		};
		_theWP = (group _x) addwaypoint [_firstWP,0];
		[(group _x), (count waypoints (group _x)) - 1] setWaypointType "CYCLE";
