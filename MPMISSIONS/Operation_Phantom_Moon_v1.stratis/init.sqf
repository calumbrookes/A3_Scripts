...
[]spawn {
waituntil{time>15};
if (isnil "headless") exitWith {hint"no HC found"};
_allAIGrps = [];
{
if (!isplayer leader _x) then {_allAIGrps pushback _x;};
}foreach allGroups;
_allAIGrps resize (ceil( (count _allAIGrps)/2)); //only assign half to HC, comment out to assign all
{
_x setGroupOwner (owner headless)
}foreach _allAIGrps;
};
};
