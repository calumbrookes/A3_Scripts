...
case "m":
{
	...
	if (_loadout <= 1) then {
		...
		(unitBackpack _unit) addMagazineCargoGlobal ["B_IR_Grenade", 3];
		(unitBackpack _unit) addMagazineCargoGlobal ["rhs_mag_mk84", 1];
		(unitBackpack _unit) addMagazineCargoGlobal ["rhs_mag_mk3a2", 1];
...
case "g":
{
	...
	if (_loadout == 0) then {
		...
		(unitBackpack _unit) addMagazineCargoGlobal [_smokegrenade, 3];
		(unitBackpack _unit) addMagazineCargoGlobal ["B_IR_Grenade", 3];
		(unitBackpack _unit) addMagazineCargoGlobal ["rhs_mag_mk84", 3];
		(unitBackpack _unit) addMagazineCargoGlobal ["rhs_mag_mk3a2", 3];
	};
	// LOADOUT: MEDIUM
	if (_loadout == 1) then {
		_unit addBackpack _bagsmall;
...
case "dm":
{
	...
	if (_loadout == 0) then {
		_unit addBackpack _bagsmall;
		(unitBackpack _unit) addMagazineCargoGlobal [_DMriflemag, 8];
		(unitBackpack _unit) addMagazineCargoGlobal ["B_IR_Grenade", 3];
		(unitBackpack _unit) addMagazineCargoGlobal ["rhs_mag_mk84", 1];
		(unitBackpack _unit) addMagazineCargoGlobal ["rhs_mag_mk3a2", 1];
		(unitBackpack _unit) addMagazineCargoGlobal [_smokegrenade, 3];
