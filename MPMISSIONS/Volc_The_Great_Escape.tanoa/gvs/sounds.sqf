...
{	
	...
	_volume = 	[_this, 5, 10] call soundsParam;
	_pitch = 	[_this, 6, 1.0] call soundsParam;
	_distance = [_this, 7, 30] call soundsParam;
	_inside = 	[_this, 8, false] call soundsParam;
...
Sound_Once3D =
{
	...
	_volume = [_this, 2, 10] call soundsParam;
	_pitch = [_this, 3, 1.0] call soundsParam;
	_distance = [_this, 4, 30] call soundsParam;
	_inside = [_this, 5, false] call soundsParam;
	_soundposition = getPos _soundsourceobject;
	playSound3D [_soundfile, _soundsourceobject, _inside, _soundposition, _volume, _pitch, _distance];
}
